
-- for Division TABLE
INSERT INTO Division VALUES ('Dhaka');
INSERT INTO Division VALUES ('Chittagong');
INSERT INTO Division VALUES ('Sylhet');
INSERT INTO Division VALUES ('Barisal');
INSERT INTO Division VALUES ('Rajshahi');
INSERT INTO Division VALUES ('Khulna');
INSERT INTO Division VALUES ('Rangpur');
INSERT INTO Division VALUES ('Mymensingh');


-- for District TABLE
INSERT INTO District VALUES ('Narayangonj', 'Dhaka');
INSERT INTO District VALUES ('Dhanmondi', 'Dhaka');
INSERT INTO District VALUES ('Gulshan', 'Dhaka');
INSERT INTO District VALUES ('Manikgonj', 'Dhaka');
INSERT INTO District VALUES ('Saver', 'Dhaka');
INSERT INTO District VALUES ('Tangail', 'Dhaka');
INSERT INTO District VALUES ('Gazipur', 'Dhaka');
insert into District values ('Krasnolesnyy', 'Dhaka');
insert into District values ('Phong Điền', 'Dhaka');
insert into District values ('Kabinda', 'Dhaka');
insert into District values ('Miandrarivo', 'Dhaka');
insert into District values ('Wangcheng', 'Dhaka');
insert into District values ('San Andres', 'Dhaka');
insert into District values ('Obsza', 'Dhaka');

insert into District values ('Pajagan', 'Chittagong');
insert into District values ('Oyem', 'Chittagong');
insert into District values ('Huizhai', 'Chittagong');
insert into District values ('Övertorneå', 'Chittagong');
insert into District values ('Pilcomay', 'Chittagong');
insert into District values ('Ehen Hudag', 'Chittagong');
insert into District values ('Salimbalan', 'Chittagong');
insert into District values ('Jargalant', 'Chittagong');
insert into District values ('Mobile', 'Chittagong');

insert into District values ('Dongzhaogezhuang', 'Sylhet');
insert into District values ('Carlton', 'Sylhet');
insert into District values ('Gzy', 'Sylhet');
insert into District values ('Dalang', 'Sylhet');
insert into District values ('Guoyuan', 'Sylhet');
insert into District values ('Urek-urek', 'Sylhet');
insert into District values ('Huilongshan', 'Sylhet');
insert into District values ('Meilin', 'Sylhet');
insert into District values ('Sedan', 'Sylhet');
insert into District values ('Brétigny-sur-Orge', 'Sylhet');
insert into District values ('Oslo', 'Sylhet');

insert into District values ('Sztum', 'Barisal');
insert into District values ('Tuzha', 'Barisal');
insert into District values ('Ash Shafā', 'Barisal');
insert into District values ('Pilar', 'Barisal');
insert into District values ('Trinidad', 'Barisal');
insert into District values ('San Juan', 'Barisal');
insert into District values ('Lagkadás', 'Barisal');
insert into District values ('La Guadalupe', 'Barisal');
insert into District values ('Plaisir', 'Barisal');
insert into District values ('Hengshan', 'Barisal');
insert into District values ('Dawang', 'Barisal');

insert into District values ('Thamaga', 'Rajshahi');
insert into District values ('Dongkengkou', 'Rajshahi');
insert into District values ('Traganón', 'Rajshahi');
insert into District values ('Gniechowice', 'Rajshahi');
insert into District values ('Omišalj', 'Rajshahi');
insert into District values ('Jomda', 'Rajshahi');
insert into District values ('Zhob', 'Rajshahi');
insert into District values ('Kousséri', 'Rajshahi');
insert into District values ('Konary', 'Rajshahi');

INSERT INTO District VALUES ('Bagerhat', 'Khulna');
INSERT INTO District VALUES ('Jashore', 'Khulna');
INSERT INTO District VALUES ('Kustia', 'Khulna');
INSERT INTO District VALUES ('Narail', 'Khulna');
INSERT INTO District VALUES ('Taligati', 'Khulna');
insert into District values ('Chaiyaphum', 'Khulna');
insert into District values ('Corupá', 'Khulna');
insert into District values ('Morelos', 'Khulna');
insert into District values ('Jishan', 'Khulna');
insert into District values ('Changfu', 'Khulna');
insert into District values ('Dinghuo', 'Khulna');
insert into District values ('Fulu', 'Khulna');

insert into District values ('Wulan', 'Rangpur');
insert into District values ('Ḩakamā', 'Rangpur');
insert into District values ('Chita', 'Rangpur');
insert into District values ('Pasadena', 'Rangpur');
insert into District values ('Siwa Oasis', 'Rangpur');
insert into District values ('Budzów', 'Rangpur');
insert into District values ('Batsari', 'Rangpur');
insert into District values ('Mazowe', 'Rangpur');
insert into District values ('Zhangtan', 'Rangpur');
insert into District values ('Ubaitaba', 'Rangpur');

insert into District values ('El Zapote', 'Mymensingh');
insert into District values ('Dārān', 'Mymensingh');
insert into District values ('Göteborg', 'Mymensingh');
insert into District values ('København', 'Mymensingh');
insert into District values ('Lalagsan', 'Mymensingh');
insert into District values ('Shatoujiao', 'Mymensingh');
insert into District values ('Akonolinga', 'Mymensingh');
insert into District values ('Ransang', 'Mymensingh');

-- for Table Client
INSERT INTO Client VALUES (1, 'Alex', 'Male', 45, null);
INSERT INTO Client VALUES (2, 'Max', 'Male', 45, 'Very week');
INSERT INTO Client VALUES (3, 'Flory', 'Female', 45, null);
INSERT INTO Client VALUES (4, 'Lorem', 'Male', 45, 'better');
INSERT INTO Client VALUES (5, 'Horton', 'Feale', 45, 'Teeth Pain');
INSERT INTO Client VALUES (6, 'Any', 'Female', 45, 'Back Pain');
INSERT INTO Client VALUES (7, 'Solax', 'Male', 45, null);
INSERT INTO Client VALUES (8, 'Dom', 'Female', 45, 'Headich');
insert into Client values (9, 'Sheffy', 'Male', 94, '');
insert into Client values (10, 'Julee', 'Female', 70, 'Suspendisse potenti.');
insert into Client values (11, 'Jennine', 'Female', 34, '');
insert into Client values (12, 'Allister', 'Male', 86, '');
insert into Client values (13, 'Lawton', 'Male', 90, '');
insert into Client values (14, 'Ingram', 'Male', 79, '');
insert into Client values (15, 'Elena', 'Female', 63, 'Vivamus vel nullapellentesque.');
insert into Client values (16, 'Tawnya', 'Female', 9, '');
insert into Client values (17, 'David', 'Male', 62, 'Quisque porta volutpat erat.');
insert into Client values (18, 'Elli', 'Female', 8, '');
insert into Client values (19, 'Clayson', 'Male', 82, '');
insert into Client values (20, 'Akim', 'Male', 96, '');
insert into Client values (21, 'Jeanna', 'Female', 40, 'Maecenas tristique, est et tempus semper.');
insert into Client values (22, 'Marthena', 'Agender', 41, '');
insert into Client values (23, 'Todd', 'Male', 39, 'Nullam orci pede, venenatis non, sodales sed, tincidunt eu, felis.');
insert into Client values (24, 'Shaylah', 'Female', 10, 'Sed accumsan felis.');
insert into Client values (25, 'Madlen', 'Female', 45, 'Etiam faucibus cursus urna.');
insert into Client values (26, 'Clio', 'Female', 89, 'Nam dui.');
insert into Client values (27, 'Julianna', 'Female', 74, '');
insert into Client values (28, 'Calypso', 'Female', 11, '');
insert into Client values (29, 'Laurene', 'Female', 37, 'Pellentesque ultrices mattis odio.');
insert into Client values (30, 'Chryste', 'Female', 95, 'Duis bibendum, felis sed interdum venenatis.');
insert into Client values (31, 'Merill', 'Male', 100, '');
insert into Client values (32, 'Berenice', 'Female', 59, '');
insert into Client values (33, 'Onfroi', 'Male', 93, '');


-- for Table Services
INSERT INTO Services VALUES (1001, 'Dr. Osim', '013325', 'Doctor', '36/37 BIT, KUET', 'Taligati', 'Khulna');
INSERT INTO Services VALUES (1002, 'Dr. Anton', '01425', 'Doctor', '334 k.b road', 'Saver', 'Dhaka');
INSERT INTO Services VALUES (1003, 'Dr. Raisa', '01525', 'Doctor', '45/37 Amlapara','Ransang', 'Mymensingh');
INSERT INTO Services VALUES (1004, 'Dr. Otaum', '01625', 'Doctor', '556 gulshan', 'Krasnolesnyy', 'Dhaka');
INSERT INTO Services VALUES (1005, 'Dr. Elli', '013234325', 'Doctor', '36/37 BIT, KUET', 'Taligati', 'Khulna');
INSERT INTO Services VALUES (1006, 'Dr. Clayson', '0145425', 'Doctor', '334 k.b road', 'Meilin', 'Sylhet');
INSERT INTO Services VALUES (1007, 'Dr. Akim', '0152545', 'Doctor', '45/37 Amlapara', 'Narayangonj', 'Dhaka');
INSERT INTO Services VALUES (1008, 'Dr. Jeanna', '01345625', 'Doctor', '556 gulshan', 'Ransang', 'Mymensingh');

INSERT INTO Services VALUES (2001, 'Sohug', '0132325', 'Ambulance', '36/37 BIT, KUET', 'Bagerhat', 'Khulna');
INSERT INTO Services VALUES (2002, 'Harun', '014251', 'Ambulance', '334 k.b road', 'Saver', 'Dhaka');
INSERT INTO Services VALUES (2003, 'Karim', '015254', 'Ambulance', '45/37 Amlapara', 'Narail', 'Khulna');
INSERT INTO Services VALUES (2004, 'Raton', '016257', 'Ambulance', '556 gulshan', 'Manikgonj', 'Dhaka');

INSERT INTO Services VALUES (3001, 'Medicine Plue', '0614251', 'Pharmacy', '334 k.b road', 'Taligati', 'Khulna');
INSERT INTO Services VALUES (3002, 'Larze Farma', '0152754', 'Pharmacy', '45/37 Amlapara', 'Narail', 'Khulna');
INSERT INTO Services VALUES (3003, 'Popular Medicine', '9016257', 'Pharmacy', '556 gulshan','Bagerhat', 'Khulna');
INSERT INTO Services VALUES (3004, 'Maa Midicine', '9016234257', 'Pharmacy', '5 no. banani','Kabinda', 'Dhaka');
INSERT INTO Services VALUES (3005, 'Pharmacy', '9234016257', 'Pharmacy', '556 narinda','Gazipur', 'Dhaka');
INSERT INTO Services VALUES (3006, 'Nhour Pharmacy', '449016257', 'Pharmacy', '77 goli','Tangail', 'Dhaka');

INSERT INTO Services VALUES (4001, 'Abu Nasir', '01527544', 'Hospital', '45/37 Amlapara', 'Taligati', 'Khulna');
INSERT INTO Services VALUES (4002, 'PG Hospital', '9016342657', 'Hospital', '556 gulshan', 'Gulshan', 'Dhaka');
INSERT INTO Services VALUES (4003, 'Medinova', '02657', 'Hospital', '556 gulshan', 'San Andres', 'Dhaka');
INSERT INTO Services VALUES (4004, 'Medistar', '23490162657', 'Hospital', '556 gulshan', 'Gulshan', 'Dhaka');
INSERT INTO Services VALUES (4005, 'Care Hospital', '862657', 'Hospital', '556 gulshan', 'Kabinda', 'Dhaka');

INSERT INTO Services VALUES (5001, 'Blood for life', '01528754', 'Blood_Bank', '478 no narinda', 'Kabinda', 'Dhaka');
INSERT INTO Services VALUES (5002, 'Safe Blood', '2343354', 'Blood_Bank', '4 no Amlapara', 'San Andres', 'Dhaka');
INSERT INTO Services VALUES (5003, 'Blood for life', '54334', 'Blood_Bank', '32 kb road', 'Manikgonj', 'Dhaka');
INSERT INTO Services VALUES (5004, 'Blood Solution', '0156728754', 'Blood_Bank', '45/37 Amlapara', 'Manikgonj', 'Dhaka');
INSERT INTO Services VALUES (5005, 'Life', '90176257', 'Blood_Bank', '556 gulshan', 'Narail', 'Khulna');



-- for Table Check_Up
insert into Check_up values (1, 2, 1003, to_date('22-Jun-22', 'dd-mon-yy'));
insert into Check_up values (2, 7, 1002, to_date('19-May-22 12:00 A.M.' , 'dd-mon-yy hh:mi A.M.'));
insert into Check_up values (3, 1, 1004, to_date('08-Apr-22 12:00 A.M.' , 'dd-mon-yy hh:mi A.M.'));
insert into Check_up values (4, 3, 1001, to_date('08-Dec-22 12:00 A.M.' , 'dd-mon-yy hh:mi A.M.'));
insert into Check_up values (5, 7, 1002, to_date('08-Jan-22 12:00 A.M.' , 'dd-mon-yy hh:mi A.M.'));

-- Just a testing quuery 
-- SELECT u.id,  c.name as client, s.name as doctor from check_up u,  Client c, Services s where c.id = u.client_id AND s.id= u.doctor_id;


-- for Table Ambulance_Book
insert into Ambulance_Book values (1, 5, 2003, '34 road, Khulna', 'Abu Nasir');
insert into Ambulance_Book values (2, 2, 2002, 'PG hospital', '44 road, Dhanmondi');
insert into Ambulance_Book values (3, 7, 2001, 'Khulna Medical', '56/33 Dakbangla');
insert into Ambulance_Book values (4, 1, 2001, '5 no road, Gulashan', 'Solimolla');
insert into Ambulance_Book values (5, 3, 2002, 'Khulna Medical', 'PG Hospital');


-- for Table Medicine_Order
insert into Medicine_Order values (1, 2, 3003, 'Napa', 10);
insert into Medicine_Order values (2, 7, 3002, 'Gendrin', 5);
insert into Medicine_Order values (3, 1, 3003, 'Flazin', 8);
insert into Medicine_Order values (4, 3, 3001, 'Amodis', 7);
insert into Medicine_Order values (5, 7, 3002, 'Aladiba', 2);


-- for Table Hospital_Book
insert into Hospital_Book values (1, 1, 4001, to_date('22-Jun-22', 'dd-mon-yy'), to_date('26-Jun-22', 'dd-mon-yy'));
insert into Hospital_Book values (2, 3, 4002, to_date('02-May-22', 'dd-mon-yy'), to_date('03-May-22', 'dd-mon-yy'));
insert into Hospital_Book values (3, 8, 4002, to_date('07-Dec-22', 'dd-mon-yy'), to_date('12-Dec-22', 'dd-mon-yy'));
insert into Hospital_Book values (4, 6, 4001, to_date('20-Mar-22', 'dd-mon-yy'), to_date('27-Mar-22', 'dd-mon-yy'));
insert into Hospital_Book values (5, 4, 4002, to_date('19-Apr-22', 'dd-mon-yy'), to_date('02-May-22', 'dd-mon-yy'));


-- for Table Blood_Bank_Info
INSERT INTO Blood_Bank_Info VALUES(5001, 20, 5, 0, 7, 58, 9, 23, NULL);
INSERT INTO Blood_Bank_Info VALUES(5002, NULL, 5, 10, 4, 8, 7, 2, 6);
INSERT INTO Blood_Bank_Info VALUES(5003, 2, 4, 6, NULL, 3, 7, 8, 1);
INSERT INTO Blood_Bank_Info VALUES(5004, NULL, 11, 3, 4, 1, 8, 22, 9);
INSERT INTO Blood_Bank_Info VALUES(5005, 3, 4, 1, 2, 6, NULL, 12, 9);
commit;


